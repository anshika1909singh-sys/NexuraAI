...
export const SkillAssessment = ({ setActiveTab }) => {
  ...
  // Calculate correct answers and domain-wise performance
  ...
  // Convert domain statistics into percentages
  const domainScores = {};
  ...
  // Temporary feedback.
  // We will replace this with AI-generated feedback later.
  const feedback = `You scored ${score}% (${correctCount}/${assessmentQuestions.length} correct).`;
      ...
      {inQuiz ? (
        ...
        <div className="p-6 sm:p-10 rounded-3xl bg-white/90 dark:bg-slate-900/90 border border-slate-200/80 dark:border-slate-800/80 shadow-2xl backdrop-blur-md space-y-6">
            ...
            {currentQ.options.map((opt, idx) => {
              ...
              return (
                <div
                  ...
                  className={`p-4 rounded-2xl border cursor-pointer transition-all flex items-center gap-3.5 ${isSelected
                      ? 'border-brand-500 bg-brand-50/80 dark:bg-brand-950/50 ring-2 ring-brand-500/30'
                      : 'border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50'
                    }`}
                >
                  <div
                    className={`w-6 h-6 rounded-full border flex items-center justify-center text-xs font-bold shrink-0 ${isSelected
          ...
          <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
            ...
            >
              <span>{currentQuestionIdx === assessmentQuestions.length - 1 ? 'Submit Assessment' : 'Next Question'}</span>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-6 relative z-10">
              <div className="space-y-2 text-center sm:text-left">
                ...
                <h3 className="text-3xl font-extrabold font-display">
                  Overall Readiness: {assessmentResult?.score ?? 0}%
                </h3>
              ...
              {Object.entries(assessmentResult?.domainScores || {}).map(([domain, score], i) => (
                <div key={i} className="space-y-1.5">
                  ...
                  <div className="w-full h-2 rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden">
                    <div
                      className={`h-full rounded-full transition-all duration-700 ${score >= 80 ? 'bg-emerald-500' : score >= 65 ? 'bg-brand-500' : 'bg-amber-500'
                        }`}
                      style={{ width: `${score}%` }}
                    />
                  </div>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="p-3 rounded-xl bg-brand-50/50 dark:bg-brand-950/30 border border-brand-500/20 text-xs text-brand-700 dark:text-brand-300 font-medium">
              Total Experience: <strong>{experienceYears || '0'} years {experienceMonths || '0'} months</strong>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-5 gap-2">
              {questionOptions.map((option) => (
                <button
                  ...
                  className={`p-3 rounded-xl border text-xs font-bold transition-all flex items-center justify-center ${numQuestions === option
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Selected: <strong>{numQuestions} questions</strong> (Estimated time: ~{Math.ceil(numQuestions * 2.5)} mins)
            </p>
          </div>
